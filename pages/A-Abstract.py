...
def load_data(address):
    ...
    CONN = sqleet.connect('./static/files/20230327.sqlite3',
                          key=slt.secrets["db_password"])
    # cursor = CONN.cursor()
    execute_sentence = "select createTime, Des, Message, Type from chathistory"
    temp = CONN.execute(execute_sentence).fetchall()
...
def show_rolling_window():
    ...
    slt.markdown(
        "> &ensp;&ensp;&ensp;&ensp;如果把每天的聊天量和星期、时间周期结合起来看，就可以得到下面这个3D柱状图，反映一周不同时间段我们的聊天偏好，如下：")
